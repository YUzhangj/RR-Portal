...
test('导出表横向展开报价项目并保留客户确认和实际生产车间', async () => {
  const row = {
    ...
    components: { injection_labor: 1.3275, dom_mat: 1.77 },
  ...
  assert.equal(sheet.getCell(4, 7).value, '啤工减税后单价');
  assert.equal(sheet.getCell(4, 8).value, '啤工减税后金额');
  assert.equal(sheet.getCell(5, 7).value, 1.3275);
  assert.deepEqual(sheet.getCell(5, 8).value, { formula: 'G5*$E5', result: 132.75 });
  assert.equal(sheet.getCell(5, 9).value.formula, 'IF($F5=0,0,G5/$F5)');
  assert.ok(Math.abs(sheet.getCell(5, 9).value.result - 0.13275) < 1e-12);
